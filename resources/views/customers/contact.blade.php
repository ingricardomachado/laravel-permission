<form action="" id="form_contact" method="POST">
    ...
    <div class="modal-body">
        <div class="row">                            
            ...
            <div class="form-group col-sm-6">
                <label>Título</label>
                {!! Form::text('occupation', $contact->occupation, ['id'=>'occupation', 'class'=>'form-control', 'type'=>'text', 'placeholder'=>'', 'maxlength'=>'100', 'required']) !!}
            </div>
            <div class="form-group col-sm-6">
...
function contact_delete(id){  
  $.ajax({
      url: `{{URL::to("customer_contacts")}}/${id}`,
      type: 'DELETE',
      data: {
        _token: "{{ csrf_token() }}", 
      },
...
function contact_CRUD(id){
    ...
    if(formulario_validado){
        ...
        $.ajax({
          url:(id==0)?'{{URL::to("customer_contacts")}}':'{{URL::to("customer_contacts")}}/'+id,
          type:'POST',
          cache:true,
          processData: false,
          contentType: false,      
