    ...
    <div class="row">
      <div class="col-9">
        <form action="#" id="form_header" method="POST">
          ...
          <input type="hidden" name="hdd_index" id="hdd_index" class="form-control" value="">
          <div class="card">
          ...
          <div class="card">
            <div class="card-body">
              <div class="row">
                ...
                <div class="col-3 input-group mb-3">
                  <div class="input-group-prepend">
                    <span class="input-group-text"><small>Precio</small>&nbsp;<span class="coin">{{ $coin }}</span></span>
                  </div>
        ...
        <div class="card">
          ...
          <div class="card-body">            
            ...
            <div class="row">
                ...
                <div class="col-12 mb-1">
                    <div class="icheck-primary d-inline">
                        {!! Form::checkbox('change_coin', null, false, ['id'=>'change_coin']) !!}
                        <label for="change_coin"><small>SIMBOLO MONEDA</small></label>
                    </div>                    
                    <input type="text" name="coin" id="coin" class="form-control" placeholder="Ej. $" value="{{ $coin }}" required="required" style="display: none;margin-top: 2mm">
                </div>
                <div class="col-12 mb-1">
                    <div class="icheck-primary d-inline">
                        {!! Form::checkbox('coin_type', null, $purchase->coin_type, ['id'=>'coin_type']) !!}
                        <label for="coin_type"><small>TIPO MONEDA</small></label>
                    </div>                    
                    <input type="text" name="coin_type_description" id="coin_type_description" class="form-control" placeholder="Ej. Dólares americanos" value="{{ $purchase->coin_type_description }}" required="required" style="{{ ($purchase->coin_type)?'display: solid':'display: none' }};margin-top: 2mm">
                </div>
            </div>
            ...
            <span>Sub Total</span>
            <h3 class="font-bold">
                <b><div><span class="coin">{{ $coin }}</span> <span id="sub_total">0,00</span></div></b>
            </h3>
            <span>Impuesto</span>
            <h3 class="font-bold">
                <div><span class="coin">{{ $coin }}</span> <span id="total_tax">0,00</span></div>
            </h3>
            <span>Total</span>
            <h2 class="font-bold">
                <b><div><span class="coin">{{ $coin }}</span> <span id="total">0,00</span></div></b>
            </h2>
              @if($type=='O')
...
$('#coin_type').on('change', function(event){
  if(event.target.checked){
    $('#coin_type_description').show();
    $('#coin_type_description').focus();
  }else{
    $('#coin_type_description').val('');
    $('#coin_type_description').hide();    
  }
});
...
$('#change_coin').on('change', function(event){
  if(event.target.checked){
    $('#coin').show();
    $('#coin').focus();
  }else{
...
function supplier_CRUD(id){
    ...
    if(formulario_validado){
        ...
        .done(function(response) {
          ...
          $('#contact').val(response.supplier.contact);
...
function update_product(){
  ...
  if(formulario_validado){
    ...
    array_types[index]=product[0].type;
...
function reset_form(){
  ...
  $('#folio').val('');
  $('#folio').hide();  
  $('#change_coin').prop('checked', false);
  $('#coin').val('');
  $('#coin').hide();
  $('#coin_type').prop('checked', false);
  $('#coin_type_description').val('');
  $('#coin_type_description').hide();
  array_descriptions=[];
  array_quantities=[];
    ...
    if(formulario_validado){
      ...
      $.ajax({
        ...
        data: {
          ...
          folio:$('#folio').val(),
          coin:$('#coin').val(),
          coin_type:($('#coin_type').is(':checked'))?1:0,
          coin_type_description:$('#coin_type_description').val(),
          ...
          ids:array_ids,
          types:array_types,
          //descriptions:array_descriptions,
          quantities:array_quantities,
          unit_prices:array_unit_prices,
          sub_totals:array_sub_totals,
